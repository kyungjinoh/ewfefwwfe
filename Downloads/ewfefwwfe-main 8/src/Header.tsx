...
import { User, LogOut, LayoutDashboard, ChevronDown, ExternalLink, MessageCircle } from 'lucide-react';
import './Header.css';
...
const Header: React.FC = () => {
  ...
  const handleDiscordClick = () => {
    window.open('https://discord.gg/cmaaNdfz7C', '_blank');
  };
      ...
      <div className="header-container">
        ...
        <nav className={`header-nav ${isMenuOpen ? 'nav-open' : ''}`}>
          ...
          {user ? (
            isMobileView ? (
              <>
                <button className="mobile-discord-button" onClick={handleDiscordClick}>
          ...
          {user ? (
            <div className="user-menu-container" ref={desktopUserMenuRef}>
              <div className="user-controls">
                <button className="discord-button" onClick={handleDiscordClick} title="Join our Discord">
                  <MessageCircle /> Join Discord
                </button>
